...
# 이미지 객체탐지 함수
def detectObject(image: Image.Image):
    img = np.array(image)       # Pillow이미지 numpy배열로 변환
    result = model(img)         # 객체탐지
    ...
    for result in result:       # 여러개 물체들을 반복
        ...
        for box, confience, class_id in zip(boxes, confiences, class_ids):
            ...
            cv2.putText(img, f'{label} {confience:.2f}', (x1,y1), cv2.FONT_HERSHEY_SIMPLEX, 0.2, (255,0,0), 1)
...
async def index():
    ...
    # result = detectObject(image)
    # result.save('result.jpg')

    return { 'message' : 'Hello FastAPI' } #, 'result': 'Image saved!'}

# ASP.NET에서 전달받은 이미지로 객체 인식, 인식 결과를 다시 ASP.NET으로 전달
